...
for j in range(int(time_step)):
    i = j+1
    local_DDS = ('DDS_0', WithUnit(float(i)*total_time/time_step, 'ms'), WithUnit(total_time/time_step, 'ms'), WithUnit(60.0+10.0*np.sin(2*np.pi*float(i)/time_step), 'MHz'), amp1, no_phase,no_freq_ramp,no_amp_ramp)
